                    ...
                    <thead>
                        <?php if (permissions("SELECT", $permissions)): ?>
                            <tr>
                                ...
                                <th><a href="<?= ROOT ?>club/members?page=1<?= $search ? "&search=$search" : "" ?>&filter=club_id<?= $filter == "club_id" && $order == "DESC" ? "&order=ASC" : "&order=DESC"?>">Club</a>
                                    <?php
                                ...
                                <th><a href="<?= ROOT ?>club/members?page=1<?= $search ? "&search=$search" : "" ?>&filter=player_id<?= $filter == "player_id" && $order == "DESC" ? "&order=ASC" : "&order=DESC"?>">Player</a>
                                    <?php
                    ...
                    <tbody>
                        <?php if (permissions("SELECT", $permissions)): ?>
                            <?php foreach ($data as $row) : ?>
                                <tr>
                                    <td class="cw-45 checkbox"><input type="checkbox" name="row[]" value="<?= $row['club_id']."-".$row['player_id'] ?>"></td>
                                    <td><?= $row['club_id'] ?></td>
                                    <td><?= $row['player_id'] ?></td>
                                    <?php if (permissions("UPDATE", $permissions)): ?>
            ...
            <div class="actions-container">
                <div>
                    <?php if (permissions("INSERT", $permissions)): ?>
                        <a href="<?= ROOT ?>club/members/form"><input type="button" name="add" value="Ajouter"></a>
                    <?php endif; ?>
                ...
                <?php if (permissions("SELECT", $permissions)): ?>
                    <div class="pages-container">
                        <a href="<?= ROOT ?>club/members?page=1<?= $search ? "&search=$search" : "" ?><?= $filter ? "&filter=$filter" : "" ?><?= $order ? "&order=$order" : "" ?>"><input <?= $current_page == 1 ? "class='active'" : "" ?> type="button" value="1"></a>
                        <?php $i = 2 ?>
                            ...
                            <?php for ($i; $i<=$last_page; $i++) : ?>
                                <?php if($i >= $current_page + 3) : ?>
                                    <?php if($current_page + 3 < $last_page) : ?>
                                        <p>...</p>
                                    <?php endif; ?>
                                    <a href="<?= ROOT ?>club/members?page=<?= $last_page ?><?= $search ? "&search=$search" : "" ?><?= $filter ? "&filter=$filter" : "" ?><?= $order ? "&order=$order" : "" ?>">
                                        <input <?= $current_page == $last_page ? "class='active'" : "" ?> type="button" value="<?= $last_page ?>">
                                    </a>
                                    <?php break; ?>
                                <?php endif; ?>
                                <a href="<?= ROOT ?>club/members?page=<?= $i ?><?= $search ? "&search=$search" : "" ?><?= $filter ? "&filter=$filter" : "" ?><?= $order ? "&order=$order" : "" ?>">
