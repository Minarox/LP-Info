                ...
                <table cellpadding="0" cellspacing="0" border="0">
                    ...
                    <?php if (permissions("SELECT", $permissions)): ?>
                        <tr>
                            ...
                            <th class="cw-90"><a href="<?= ROOT ?>newspapers/news?page=1<?= $search ? "&search=$search" : "" ?>&filter=id<?= $filter == "id" && $order == "DESC" ? "&order=ASC" : "&order=DESC"?>">Id</a>
                                <?php
                            ...
                            <th><a href="<?= ROOT ?>newspapers/news?page=1<?= $search ? "&search=$search" : "" ?>&filter=date<?= $filter == "date" && $order == "DESC" ? "&order=ASC" : "&order=DESC"?>">Date</a>
                                <?php
                            ...
                            <th><a href="<?= ROOT ?>newspapers/news?page=1<?= $search ? "&search=$search" : "" ?>&filter=name<?= $filter == "name" && $order == "DESC" ? "&order=ASC" : "&order=DESC"?>">Name</a>
                                <?php
                    ...
                    <?php if (permissions("SELECT", $permissions)): ?>
                        <?php foreach ($data as $row) : ?>
                            <tr>
                                ...
                                <td><?= $row['name'] ?></td>
                                <?php if (permissions("UPDATE", $permissions)): ?>
                                    <td class="cw-100 action"><a href="<?= ROOT ?>newspapers/news/form?id=<?= $row['id'] ?>"><input type="button" value="Editer"></a></td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
            ...
            <div class="actions-container">
                <div>
                    <?php if (permissions("INSERT", $permissions)): ?>
                        <a href="<?= ROOT ?>newspapers/news/form"><input type="button" name="add" value="Ajouter"></a>
                    <?php endif; ?>
                ...
                <?php if (permissions("SELECT", $permissions)): ?>
                    <div class="pages-container">
                        <a href="<?= ROOT ?>newspapers/news?page=1<?= $search ? "&search=$search" : "" ?><?= $filter ? "&filter=$filter" : "" ?><?= $order ? "&order=$order" : "" ?>"><input <?= $current_page == 1 ? "class='active'" : "" ?> type="button" value="1"></a>
                        <?php $i = 2 ?>
                            ...
                            <?php for ($i; $i<=$last_page; $i++) : ?>
                                <?php if($i >= $current_page + 3) : ?>
                                    <?php if($current_page + 3 < $last_page) : ?>
                                        <p>...</p>
                                    <?php endif; ?>
                                    <a href="<?= ROOT ?>newspapers/news?page=<?= $last_page ?><?= $search ? "&search=$search" : "" ?><?= $filter ? "&filter=$filter" : "" ?><?= $order ? "&order=$order" : "" ?>">
                                        <input <?= $current_page == $last_page ? "class='active'" : "" ?> type="button" value="<?= $last_page ?>">
                                    </a>
                                    <?php break; ?>
                                <?php endif; ?>
                                <a href="<?= ROOT ?>newspapers/news?page=<?= $i ?><?= $search ? "&search=$search" : "" ?><?= $filter ? "&filter=$filter" : "" ?><?= $order ? "&order=$order" : "" ?>">
